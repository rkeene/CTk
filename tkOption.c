 ...
 *	adapted from tk4.2b1 by JuanJo <jjciarla@raiz.uncu.edu.ar>
 *
				 ...
				 * 0 and TK_MAX_PRIO. */
{
	...
	while (*src != ':') {
	    if ((*src == '\0') || (*src == '\n')) {
		char buffer[100];
		sprintf(buffer, "missing colon on line %d",
			lineNum);
	...
	*dst = '\0';
	...
	if (*src == '\0') {
	    char buffer[100];
	    sprintf(buffer, "missing value on line %d", lineNum);
	    Tcl_SetResult(interp,buffer,TCL_VOLATILE);
	    return TCL_ERROR;
	}
	...
	while (*src != '\n') {
	    if (*src == '\0') {
	        char buffer[100];
		sprintf(buffer, "missing newline on line %d",
			lineNum);
	        Tcl_SetResult(interp,buffer,TCL_VOLATILE);
		return TCL_ERROR;
	    }
				 ...
				 * is an ancestor of the desired leaf. */
{
    ...
    if (winPtr->parentPtr && winPtr->parentPtr->parentPtr) {
